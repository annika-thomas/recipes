/**
 * The shape of a recipe, and the one place that decides what's valid.
 *
 * Both the importer (which gets its JSON from Claude) and the editor (which
 * gets it from a form) funnel through `normaliseRecipe`, so a recipe scraped
 * off a reel and one typed by hand are indistinguishable once stored.
 */

import { str, text, num, int } from './http.js';
...
/**
 * Ingredient-name normalisation is shared with the browser — one copy, in
 * public/js/util/match.js, so the dots on the ingredient list and the pantry
 * matcher can never drift apart. Wrangler bundles it into the Worker.
 */
